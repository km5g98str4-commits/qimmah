...
const PRESETS: { key: string; label: string; yaw: number }[] = [
  { key: 'front', label: 'أمامي', yaw: 0 },
  { key: 'left', label: 'جانب', yaw: Math.PI / 2 },
  { key: 'back', label: 'خلفي', yaw: Math.PI },
  { key: 'right', label: 'جانب آخر', yaw: -Math.PI / 2 },
]
...
export function BodyModel3D({ className }: { className?: string }) {
  ...
  const [angleLabel, setAngleLabel] = useState('أمامي')
  ...
  const syncAngleLabel = useCallback(() => {
    ...
    const label =
      deg < 45 || deg >= 315 ? 'أمامي' : deg < 135 ? 'جانب' : deg < 225 ? 'خلفي' : 'جانب آخر'
  ...
  const goTo = (yaw: number) => {
    ...
    setAngleLabel(PRESETS.find((p) => p.yaw === yaw)?.label ?? angleLabel)
    requestFrame()
  }
  ...
  const caption = selected
    ? sel && sel.sets > 0
      ? `${muscleLabelAr(selected)} · ${sel.sets} من ${selTarget} مجموعة هذا الأسبوع`
      : `${muscleLabelAr(selected)} · ما سجّلت لها شيء بعد`
    : trainedCount > 0
    ...
    <div ref={wrapRef} className={cn('card p-5', className)}>
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-black text-ink-900">مجسّم عضلاتك</p>
            <p className="text-[11px] font-bold text-ink-400">
              هذا الأسبوع · {genderLabel}
              {mode === '3d' ? ` · ${angleLabel}` : ''}
            </p>
          </div>
        </div>
        ...
        <div className="mt-3 rounded-xl border border-line bg-page p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="rounded-lg bg-primary-soft px-2 py-0.5 text-[11px] font-black text-primary-c">
              {muscleMap[selected]?.size === 'large' ? 'عضلة كبيرة' : 'عضلة صغيرة'}
            </span>
          </div>
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-ink-500">
            {sel.sets > 0
              ? `${sel.sets} مجموعة من ${selTarget} مستهدفة · ${sel.exercises} تمرين مختلف`
              : 'ما لمستها هذا الأسبوع — أضف لها تمرينًا في خطتك.'}
          </p>
        </div>
      )}

...
/** اسم اليوم المختصر بالعربية من ختم YYYY-MM-DD. */
function shortDay(date: string): string {
  const d = new Date(`${date}T12:00:00`)
  try {
    return new Intl.DateTimeFormat('ar', { weekday: 'short' }).format(d)
  } catch {
    return ['أحد', 'إثن', 'ثلا', 'أرب', 'خمي', 'جمع', 'سبت'][d.getDay()]
  }
}

export function StepCounterCard({ className }: { className?: string }) {
    ...
    <div className={cn('card p-5', className)}>
      ...
      <div className="mb-4 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-sm font-black text-ink-900">خطوات اليوم</p>
            <p className="text-[11px] font-bold text-ink-400">إدخال يدوي</p>
          </div>
        </div>
        ...
        {editingGoal ? (
          <div className="flex items-center gap-1">
            <input
              ...
              aria-label="الهدف اليومي للخطوات"
      ...
      <div className="flex flex-col items-center">
        <div className="relative grid place-items-center">
          ...
          <div className="absolute flex flex-col items-center">
            ...
            <span className="text-[11px] font-bold text-ink-400">من {goal.toLocaleString('en-US')}</span>
        ...
        <p className="mt-2 text-center text-xs font-bold text-ink-500">
          {reached ? '🎉 وصلت هدفك اليوم — أحسنت!' : `باقي ${remaining.toLocaleString('en-US')} خطوة للهدف`}
        </p>
      </div>
      ...
      <div className="mt-5 border-t border-line pt-4">
        <p className="mb-2 text-[11px] font-bold text-ink-500">آخر ٧ أيام</p>
        <div className="flex items-end justify-between gap-1.5" style={{ height: 64 }}>
          {week.map((d, i) => {
            ...
            return (
              <div key={d.date} className="flex flex-1 flex-col items-center justify-end gap-1" title={`${d.steps.toLocaleString('en-US')} خطوة`}>
                <div
                  className={cn(
                  ...
                  style={{ height: h }}
                />
                <span className={cn('text-[9px]', isToday ? 'font-black text-ink-700' : 'text-ink-400')}>{shortDay(d.date)}</span>
              </div>
            )
          })}
        </div>

// ⛔ DEAD SURFACE — مبنيّ، ومُثبَت، وغير موجَّه.
//
// محرّك التخصيص (`src/lib/personalization/` عدا `experience.ts` و`types.ts`)
// **لا يبلغه التطبيق**: المستهلك الحيّ الوحيد هو `src/lib/onboardingV2Flow.ts`
// وهو يستورد هذين الملفّين فقط. الأسئلة التي يراها المستخدم تأتي من
// `onboardingV2Flow.ts` — نظام منفصل تمامًا عن هذا المحرّك.
//
// ⚠️ ومع ذلك يستهلك `test:personalization` (داخل البوابة) تسعة عشر ملفًّا من هنا
// بتسعين فحصًا. خضرتها **لا تعني** أن شيئًا من هذا يصل مستخدمًا (GOV-002 §7).
//
// لا يُحذف في هذه الموجة. الوسم غرضه منع وكيل قادم من البناء فوقه ظنًّا أنه حيّ.

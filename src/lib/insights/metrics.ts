...
export function computeMuscleSplit(input: InsightInput, minSessions = 2): MuscleSplitMetric {
  ...
  // مجموعات أساسية يُتوقّع لمسها أسبوعيًا؛ ما لم يُلمَس = ناقص (تطابق MUSCLE_GROUPS).
  const CORE = ['الصدر', 'الظهر', 'الأرجل', 'الأكتاف', 'الذراع', 'الكور']
  const undertrained = CORE.filter((g) => !counts.has(g))
  ...
  for (const s of input.sessions) {
    ...
    for (const ts of s.topSets) {
      ...
      if (gapPct >= THRESHOLDS.prNearMinPct && gapPct <= THRESHOLDS.prNearMaxPct) {
        if (!best || gapPct < best.pct) best = { name: rec.nameAr || ts.nameAr, pct: gapPct }
      }
    }
  }
  if (!best) return { key: 'pr', status: 'abstain', reason: 'insufficient', exerciseNameAr: null, nearestPct: null }
  return { key: 'pr', status: 'ok', exerciseNameAr: best.name, nearestPct: Math.round(best.pct * 10) / 10 }
}

...
import { STORE_DEFS, ownerToken, readRaw, sweepUnregistered, type StoreDef } from './registry'
import {
  BUNDLE_KIND,
...
export function buildExportBundle(uid: string | null | undefined, now: Date = new Date()): PortabilityBundle {
  ...
  for (const def of STORE_DEFS) {
    const value = captureStore(def, uid)
    if (value === undefined) continue
    stores[def.id] = value
    counts[def.id] = def.count(value)
  }
  const unregistered = sweepUnregistered(uid)

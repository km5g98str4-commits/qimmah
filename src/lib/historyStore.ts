...
export interface NutritionLog {
  ...
  /**
   * هدف السعرات **الأساسي** الذي كان ساريًا في هذا اليوم — يُسجَّل لحظة عرضه على
   * المستخدم (`nutritionCarryover.recordDayBaseTarget`). ضروري لترحيل الفائض:
   * «هل تجاوز أمس هدفه؟» سؤال عن هدف **الأمس** لا هدف اليوم، وتغيير الخطة يجعل
   * الاثنين مختلفين. يومٌ بلا هذا الحقل ⇒ لا يُرحَّل منه شيء (لا هدف مُختلَق).
   */

...
/**
 * هل التخزين **متاح للقراءة** أصلًا؟ `false` يعني محجوبًا (`SecurityError`
 * من سياسة متصفّح/مؤسسة) أو غائبًا (SSR) — لا ممتلئًا.
 *
 * [SOVEREIGN-RECOVERY-001] لماذا هذه غير `isStorageWritable`: تلك تكتب مفتاح
 * تحقّق، فتُرجع `false` أيضًا حين تكون الحصّة ممتلئة والتخزين **مقروء تمامًا**.
 * وتمييز «محجوب» عن «فارغ» يحتاج فحص القراءة وحده — وإلا صار المستخدم صاحب
 * الحصّة الممتلئة «مستخدمًا جديدًا» في عين طبقة الاسترجاع.
 */

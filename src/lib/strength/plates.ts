...
function isValidConfig(v: unknown): v is PlateConfig {
  ...
  if (typeof c.barKg !== 'number' || c.barKg <= 0) return false
  if (!Array.isArray(c.plates) || c.plates.length === 0) return false
  return c.plates.every((p) => p && typeof p.kg === 'number' && p.kg > 0 && Number.isInteger(p.pairs) && p.pairs >= 0)
}

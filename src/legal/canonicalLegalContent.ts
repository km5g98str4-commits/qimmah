...
function privacy(lang: Lang): LegalDocument {
  ...
  if (lang === 'en') return {
    ...
    sections: [
      ...
      { heading: '6. Storage location and transfers', paragraphs: [`Cloud data is processed and stored in: ${configured(c.dataRegionEn, 'dataRegionEn')}. Local use stays on your device with no transfer.`] },
      { heading: '7. Control, export, and deletion', paragraphs: ['Local data remains until you clear it or remove the app. Available settings let you export local data and request account deletion. A deletion failure must not be presented as success.'] },
  ...
  return {
    ...
    sections: [
      ...
      { heading: '٦. موقع التخزين والنقل', paragraphs: [`تُعالَج البيانات السحابية وتُخزَّن في: ${configured(c.dataRegionAr, 'dataRegionAr')}. ويبقى الاستخدام المحلي على جهازك بلا نقل.`] },
      { heading: '٧. التحكم والتصدير والحذف', paragraphs: ['تبقى البيانات المحلية حتى تمسحها أو تحذف التطبيق. تتيح الإعدادات المتوفرة تصدير البيانات المحلية وطلب حذف الحساب. لا يجوز عرض نجاح إذا فشل الحذف.'] },
...
function terms(lang: Lang): LegalDocument {
  ...
  if (lang === 'en') return {
    ...
    sections: [
      ...
      { heading: '8. Availability, estimates, and termination', paragraphs: ['The service and its estimates are provided without a promise of uninterrupted availability or a particular result. You may request account deletion. Access may be suspended for a material breach of these Terms.'] },
      { heading: '9. Governing terms and contact', paragraphs: [`Governing law and venue: ${configured(c.governingVenueEn, 'governingVenueEn')}. Provider: ${configured(c.controllerNameEn, 'controllerNameEn')}. Contact: ${configured(c.contactEmail, 'contactEmail')}.`] },
    ],
  }
  return {
    ...
    sections: [
      ...
      { heading: '٨. التوفر والتقديرات والإنهاء', paragraphs: ['لا تعد الخدمة أو تقديراتها بتوفر بلا انقطاع أو نتيجة معينة. يمكنك طلب حذف حسابك. وقد يوقف الوصول عند مخالفة جوهرية للشروط.'] },
      { heading: '٩. النظام والتواصل', paragraphs: [`النظام الحاكم والاختصاص: ${configured(c.governingVenueAr, 'governingVenueAr')}. مقدم الخدمة: ${configured(c.controllerNameAr, 'controllerNameAr')}. التواصل: ${configured(c.contactEmail, 'contactEmail')}.`] },
    ],
  }
}

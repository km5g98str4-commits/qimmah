    ...
    <div className="mt-3 rounded-2xl p-4" style={{ background: FOCUS.card, border: `1px solid ${FOCUS.line}` }} role="group" aria-label={t('حاسبة الأقراص', 'Plate calculator')}>
      ...
      {load.reachable ? (
        ...
        <div className="mt-1 text-sm font-bold">
          <p className="tabular-nums" style={{ color: FOCUS.ember }}>
            {t('غير قابل تمامًا — أقرب', 'Not exact — nearest')} {toAr(load.achievedKg, lang)} {t('كجم', 'kg')} ({load.deltaKg > 0 ? '+' : ''}{toAr(load.deltaKg, lang)})
          </p>
          {load.suggestion && (

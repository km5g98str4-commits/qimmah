...
|---|---|---|---|---|
| **C-1** | **Lawful basis / consent — analytics** | **Closed structurally in [CTO-71].** The transmit-capable layer was deleted; usage events never leave the device, so no consent basis for transmission is required. | **Closed** | Re-open only if a transmission path is ever reintroduced — which `test:analytics` blocks by named check. |
...
| **R-4** | **Right to object / restrict** | No in-app objection/restriction mechanism beyond deletion. | **Gap (minor)** | State in the Privacy Policy that requests go to `qimmah.support@gmail.com`; handle manually. |

...
```bash
node scripts/run-strength-proof.mjs      # 33 checks — see §Tests
```
...
## Tests (`scripts/strength-proof.ts` — 33 checks)
...
- **Edge armor:** hostile pair counts are rejected; `actualReps=0` never falls back to target reps or creates an e1RM/PR.

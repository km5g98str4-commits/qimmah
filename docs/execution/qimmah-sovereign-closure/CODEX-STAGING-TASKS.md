...
> **وكل حزمة صارت تُعيد صفّ حالتها** (`bundle | registered | expected | status`)
> — فإعادة أي لصقة تُقرأ عبر واجهة الإدارة بلا اعتماد على الإشعارات.
...
**المتوقَّع:** صفٌّ واحد يحمل الحكم كاملًا:

```
verdict | passed | failed | leftover_users | leftover_codes | total_users
PASS    |   9    |   0    |       0        |       0        |      0
```

ومعه عمود `checks` فيه التسعة بأسمائها — فيُعرَف **أيّها** سقط لا أنّ واحدًا سقط.

⚠️ **لا يحتاج محرّر SQL ولا تسجيل دخول.** الحكم يعود صفًّا، فيصل عبر واجهة
الإدارة كما يصل عبر المحرّر. (أوّل صيغة كتبت نتائجها بـ`RAISE NOTICE` وحدها،
فشُغّلت عبر الواجهة **وضاعت الفحوص التسعة** — أُصلح في المنبع.)

**أرسل الصفّ كما هو.** و`verdict = FAIL` = توقّف وأرسله بلا إصلاح.

---

## ٣٫٦. مسبار السطح العامّ — الطبقة التي لم يمسّها الفحص السلوكي

الفحص السلوكي جرى عبر **واجهة الإدارة**، أي بدور **مالك الجداول**. ولا هجرة
عندنا تكتب `force row level security`، والمالك يتجاوز RLS. فما أُثبت هو
**منطق الدوالّ**، لا طبقة المنح وRLS وPostgREST.

**والفرق بينهما هو الفرق بين «المنطق صحيح» و«لا يمكن الالتفاف عليه».**

```bash
SUPABASE_URL='https://odpkvswfiihrkglgfghd.supabase.co' \
SUPABASE_ANON_KEY='<المفتاح العامّ نفسه>' \
npm run staging:probe
```

يسأل: ماذا يبلغ متصفّحٌ يحمل مفتاح `anon` وحده؟ الجداول · الدوالّ الإدارية ·
دوالّ العميل بلا مصادقة · المسار القديم المسحوب.

⛔ **لا تمرّر `service_role` هنا إطلاقًا** — والمسبار يرفضه من نفسه: السرّ
يقلب كل رفضٍ قبولًا فيصير تضليلًا لا إثباتًا.

✅ **ولا يكتب شيئًا** — كل نداء فيه قراءةٌ أو نداءٌ يُنتظَر رفضه. فيجوز
تشغيله بعد إنشاء حسابك، بخلاف §٣٫٥.

> **وإن أعلن أنه «أعمى» وتوقّف** فذلك سلوكه الصحيح لا عطل: ضابطه الموجب يقارن
> مفتاحًا صحيحًا بآخر مهشَّم، فإن لم يفترقا فكل رفضٍ بعدهما لا يُثبت حراسة.
> أرسل ما طبع كما هو.

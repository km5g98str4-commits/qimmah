...
**وكل حزمة تُعيد صفًّا يقول ماذا فعلت** — لا إشعارًا وحده:

```
bundle | registered | expected | status
2/6    |     5      |    5     |  OK
```

`INCOMPLETE` ⇒ توقّف وأرسل الصفّ. (كانت الحزم ١-٥ بلا `select` إطلاقًا، فعبر
واجهة الإدارة يصل `201` وناتجٌ فارغ — الفشل يظهر، لكن **النجاح لا يُميَّز من
اللاشيء**.)

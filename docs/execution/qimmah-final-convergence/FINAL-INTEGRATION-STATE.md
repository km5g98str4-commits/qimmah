...
# 5. النتيجة — ما استُوعب فعلًا

الحارات **E** و**F** كانتا مستوعَبتين في `1614d41` قبل أن تبدأ هذه الموجة (٠ متبقٍّ).
والمتبقّي أُدمج واحدًا واحدًا، كلٌّ بتنبّؤ `merge-tree` قبله وفحص قيم بعده (§4.1):

| # | الالتزام | المصدر | التعارض | الحسم |
|---|---|---|---|---|
| FINAL-001 | `863e540` | حارة B | **لا شيء** | دمج مباشر |
| FINAL-002 | `d405b38` | حارة A | `evidence/latest.json` | أبقيتُ **الأحدث والأوسع** (١٠ أطقم/٤٤٥ مقابل ٢/٨٠)، وكلاهما `provisional` ويُعاد توليده |
| FINAL-003 | `dfde745` | حارة C | `package.json` | **اتحاد** — أُضيف `test:e2e:exercise-detail` |
| FINAL-004 | `e12bad0` | حارة D | `package.json` | **اتحاد** — أُضيف `food:emit` · `test:food-catalog` · `test:e2e:food-catalog`، و`test:food-catalog` إلى البوابة |
| FINAL-006 | `0731bf2` | حارة A | **لا شيء** | REL-001 — سياسة أرقام واحدة |
| FINAL-011 | `54ad599` | حارة A | `package.json` | اتحاد — **أضاف صفرًا**: نفس الربط الذي نُفِّذ في FINAL-007 |

**ولا سكربت واحد فُقد** — مطبوعًا بعد كل حسم، لا مفترضًا.

## إصلاحات التقارب — ما وجدتُه أنا لا الحارات

| # | العطل | كيف ظهر |
|---|---|---|
| FINAL-005 | `--skip-build` يقبل أرتيفكتًا بائتًا ويختمه بالرأس الحالي | قراءتي للحارس (§15) |
| FINAL-007 | `test:numeral-policy` غير مربوط | واجب المنسّق الذي أرجأته الحارة صراحةً (§1.4) |
| FINAL-008 | BUG-029..032 دخلت السجلّ بلا رصد | **البوابة كشفته في موجته** — ٣٢ موثّقًا مقابل ٢٨ مرصودًا |
...
الحارس حيًّا على الضيف **وميتًا على الشريحة المدفوعة بالضبط**. وكان خاطئًا في
الاتجاه الآخر أيضًا: علم جهاز من ضيف سابق يقفل حسابًا جديدًا فيمنع إكمالًا أوّلَ
مجانيًا بحقّ. أُثبت بـ٩ فحوص على تخزين حقيقي، وأحمرُه مُعاد إنتاجه (٤ فشل/خروج ١).

## البوابة على الرأس النهائي

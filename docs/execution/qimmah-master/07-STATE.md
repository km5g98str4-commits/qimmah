...
### SALLA-V1 — جاهز حتى حدّ يد المؤسس
- `docs/product/SALLA-V1-EXECUTION.md` + `scripts/salla/format-codes.mjs`. الاحتياطي ١٠٠٠/١/٠ (قراءة فقط).
- إصدار المخزون بيد المؤسس حصرًا (`require_founder`) — مقصود.
- **قائمة المؤسس بلا قرارات تقنية:** `docs/product/SALLA-FOUNDER-CHECKLIST.md` (أ منتج غير منشور ·
  ب `SALLA-TEST-001` ٥ أكواد وشراء واحد واسترداد ثانٍ يفشل · ج `SALLA-LAUNCH-001` ٢٠٠). المطلوب
  الوحيد منه قبل التحويل: لقطة شاشة/قالب صفحة «رفع دفعة» في سلة.
- **`COMMERCE_READY = NO`** حتى يتمّ ب وج بيد المؤسس. `WEB_CORE_READY = YES` · `RELEASE_READY = CONDITIONAL`.
...
```
main       : (يُملأ بعد الترقية) ← 59106ad
الترقيتان : FOOD-OFF-SA-001 (بيانات الطعام) ثم MOBILE-SHELL-001 (القشرة) — إيداع مستقلّ لكل بند
```

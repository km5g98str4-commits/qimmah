...
الحكم     : RELEASE_BLOCKED — ثلاثة P1 أُصلحت في الفرع ولم تُطبَّق على الإنتاج بعد، وواحد تشغيلي بيد المؤسس
```
...
### ما ينتظر المؤسس قبل أي إطلاق عامّ

1. تطبيق الحزمة ٩ (`20260906120001` · `20260906120002`) على staging ثم الإنتاج.
2. الخطّة Free على الإنتاج: **لا نسخ احتياطي ولا PITR**، وSMTP المدمج محدود ببضع رسائل في الساعة بينما كل RPC تجاري يشترط بريدًا مؤكَّدًا — ترقية الخطّة و/أو SMTP مخصّص **قبل** يوم الإطلاق.
3. تفعيل «Leaked password protection» في لوحة Auth (تحذير المستشار).
4. `admin_grant_premium` (مسار سلة V2 غير المنشور) لا يربط صفّ الشراء — دَين مسمًّى قبل تفعيل الـwebhook.

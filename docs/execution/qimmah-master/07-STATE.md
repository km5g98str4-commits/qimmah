...
**Last updated:** 2026-09-15 · **by:** [NUTRITION-UNIFY-001]

---

## 0-ك. توحيد شاشة التغذية + تاريخ يومي + ترحيل الفائض — [NUTRITION-UNIFY-001] (١٥ سبتمبر)

```
الفرع     : claude/qimmah-nutrition-consistency-p3gdit (من main @ 0018700)
السبب     : لقطتا المؤسس — مستخدمان على نفس النسخة يريان بنيتَي تغذية مختلفتين
الجذر     : NutritionView كانت تتفرّع على nutritionPlan.style: 'meal_suggestions'
            ⇒ أقسام وجبات · غيرها ⇒ مسجّل واحد مسطّح بلا كمية ولا تعديل.
            والقيمة تأتي من سؤال «النية» في OnboardingV2 — إجابةٌ تبدّل المعمار.
البوابة   : typecheck · lint · build · test:gate الأربع خضراء بعد npm ci
الإثباتات : test:nutrition-canonical (86) في البوابة ·
            scripts/e2e/nutrition-canonical-live.mjs (60 في المتصفّح) في CI
التراجع   : git revert لالتزامات الفرع — لا هجرة قاعدة، ولا مفتاح تخزين يُحذف
```

### ما تغيّر
- **الفرع البنيوي أُزيل.** أقسام الوجبات (فطور/غداء/عشاء/سناك) لكل مستخدم.
  `style` يبقى **مخزَّنًا وحيًّا** في توليد الخطة وفي شاشة «قيمتك» — تفضيل لا معمار.
  **لا هجرة بيانات ولا مسح تفضيل:** الإصلاح في مسار **القراءة** وحده.
- **تصفّح الأيام** في الشاشة القانونية فوق الدفتر المؤرَّخ القائم
  (`qimmah:nutritionHistory:v1`): كل يوم بوجباته وكمياته وماكروزه وماءه وهدفه.
  اليوم الماضي **يُقرأ ويُعدَّل ويُحذف منه** ولا يُضاف إليه (السبب معلَن في الشاشة).
- **ترحيل فائض السعرات** — مفتاح **مطفأ افتراضيًا** (`qimmah:nutritionCarryover:v1`،
  مفتاحه المالك، داخل التصدير/الاستيراد). الخصم **مشتقّ لا مخزَّن**، فتعديل طعام
  الأمس يصحّح هدف اليوم من تلقائه. الأرضية `calculators.calorieFloor` **مستوردة
  لا معادة التعريف**.

### ما يحتاج قرار مؤسس (معزول، لا يحجب شيئًا)
1. **ترحيل العجز** (أكلتَ أقلّ ⇒ خذ الفرق غدًا) **غير منفَّذ** — معزول في
   `CARRYOVER_DEFICIT_POLICY = 'ignore'`. الاسم والمثال يتكلّمان عن الفائض وحده،
   وترحيل العجز سلوك صحّي مختلف.
2. **«اقتراحات أكل جاهزة» لا تُعرض في أي سطح حيّ.** `nutritionPlan.meals` المولَّدة
   يقرؤها `NutritionV2.tsx` **التوأم الميت** وحده. الوعد في الإعداد قائم والعرض
   غائب — فجوة صدق سابقة لهذه الموجة، تُجدول مستقلّة.
3. **إضافة طعام ليوم ماضٍ** غير مدعومة (التعديل والحذف مدعومان). كاتب ماضٍ جديد
   له أثر مزامنة يستحق موجته.
...
الرؤية    : الرابط المباشر يحوّل الزائر المجهول إلى جذر المتجر في ٤ قراءات متتالية (أوّلها طلب بارد) — HIDDEN=YES مؤكَّد، والشراء يحتاج فتح الرؤية لحظتَه
الترقية   : main @ b7bf3a1 (CI #678 أخضر على d961d96؛ ٧ التزامات، لا هجرة)
التراجع   : git revert c4adf99..b7bf3a1 — واجهة وإعداد ووثائق وسكربت CI فقط؛ لا مسّ للقاعدة ولا للمخزون
...
```
الفرع     : claude/qimmah-production-readiness-13cyt7
الترقية   : main @ c4adf99 (CI #670 أخضر على bc1b022؛ ٢٢ التزامًا مقطوفة، لا هجرة)
التراجع   : git revert للالتزامات الـ٢٢ — بيانات وواجهة فقط؛ مفتاح qimmah:personalFoods:v1 قائم من قبل ولا يُمسّ
القاعدة   : ما يُسجَّل مرّة يُطلب غدًا — الأكلة المخصّصة مخزنٌ شخصي لا قيد يوم؛ ومطاعم بمصدر رسمي أو لا شيء
```

### أكلاتي — الأطعمة المخصّصة صارت قابلة لإعادة الاستعمال
- **الكسور:** حقول المخصّص `type="text" inputMode="decimal"` (لا `type=number` الذي يُفرِّغ «٢٧٫٥»)، والتعقيم
  يقبل «.» و«,» و«٫» والأرقام العربية؛ القيم تُحفظ بخانة عشرية (300.5 · 27.5 · 8.3).
- **المخزن:** امتداد `qimmah:personalFoods:v1` القائم (موسوم بالمالك، ضمن التصدير) لا مخزن موازٍ —
  `savePersonalFood` (اسم إلزامي · تكرار الاسم المطبَّع يُحدَّث لا يُكرَّر · السقف ٢٠٠ مرفوض مسمًّى ·
  كتابة عبر `safeStorage` بنتيجة `storage` لا `ok`) · `markPersonalFoodUsed` · `searchPersonalFoods` ·
  `personalFoodPortion` (الضرب في نقطة واحدة) · الكارب/الدهون اختياريتان (الفارغ غائب لا صفر).
- **الواجهة (`QuickMealLogger`):** تبويب ثالث «أكلاتي» (سجّل بمضاعف حصص · تعديل · حذف بتأكيد على خطوتين) ·
  الحفظ افتراضيًا من «إضافة مخصّصة» بخيار معلَن · نتائج البحث تعرض أكلات المستخدم أوّلًا بشارة «من أكلاتي».
- **الإثبات:** `test:personal-foods` (٥١ على المخزن + فحص بنيوي للشاشة) في البوابة ·
  `test:e2e:personal-foods` (٢٧ في المتصفّح: إنشاء بكسور → يوم جديد → أكلاتي → ١٫٥ حصة → بحث → تعديل → حذف → EN) في CI.

### تغطية المطاعم
| السلسلة | قبل | بعد | المصدر والتصنيف |
|---|---|---|---|
| أفران الحطب (الحطب فودز) | ٠ باسمها (٤٤ عامّة «أفران») | **١٢٦** | ملصقات المنتجات الرسمية على `alhatab.com.sa` (لكل 100غ × وزن العبوة) ⇒ OFFICIAL_LOCAL كاملة الماكروز؛ ٢٢ محجورة مسمّاة (بلا وزن · ملصق ناقص · حصة ≠ 100غ) + ٣ خارج أتواتر ±٣٠٪ |
| شاورمر | ٣٩ | **٤٠** (المنيو الرسمي كاملًا) | SFDA: سعرات+بروتين رسمية، كارب/دهون غير متوفّرة (OFFICIAL_PARTIAL) |
| ماكدونالدز | ١٥ | **٢٨** | USDA SR Legacy (USDA_MEASURED، سوق أمريكي موسوم) — الحاسبة الرسمية السعودية `mcdonalds.com/sa/…/nutrition-calculator` موجودة لكنها تحجب عدّاء CI (HTTP/2 مقطوع + curl مرفوض)؛ ١٣ صنفًا لا سجلّ لها في USDA (McDouble · Big Tasty · الشيكات · Spicy McChicken) |
- بطاطس ماكدونالدز اليدوية حلّ محلّها سجلّ USDA بثلاثة أحجام؛ `test:food-db` مثبَّت على ٩٩ يدوية / ٢٩٠ رسمية.
- الجالب `fetch-chain-evidence.mjs`: `browserLike` (HTTP/1.1 + هوية كروم معلَنة) · `captureJson` · `waitMs`؛ السجلّ
  نُظِّف من ٦٣ بذرة ميتة (موثَّقة في `_retired`) وأُضيفت ٢٣٠ صفحة منتج للحطب.

### ما لم يُفعل ولماذا
- **حاسبة ماكدونالدز السعودية** (المصدر الرسمي الأدقّ للوصفة المحلية) محجوبة عن عدّاء GitHub وعن بيئة الوكيل —
  تحتاج قراءة من جهاز المؤسس (لقطة/تصدير) أو مسار شبكة آخر. البديل المعتمد الآن USDA موسومًا بسوقه.
- منصّات التوصيل (هنقرستيشن: Cloudflare 403 · نينجا: متجر داكن بلا سعرات) لا تصلح مصدرًا.

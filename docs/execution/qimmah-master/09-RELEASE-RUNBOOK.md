...
#### مسألتان حُسمتا بالقياس — لا تُعاد فتحهما بلا دليل جديد

1. **أمر بناء Cloudflare Pages سليم.** أثار إصلاحُ CI شكًّا مشروعًا: لو كان
   المضيف يبني بـ`vite build` بدل `npm run build` لخرج الإنتاج **بلا أصول
   البحث المولَّدة** (٦٠ ألف سجلّ). **قِيس فحُسم:** معاينة الفرع — وتُبنى بنفس
   خطّ Pages — تخدم `/food/search/manifest.json` بـ200، وهو ملفٌّ **لا يولّده
   إلا `npm run build`** ولا يُلتزم في git. فالمضيف يشغّل السكربت الصحيح،
   والعطل كان **في CI وحده**. (قِيس على `1d5dae9`.)
2. **ما لا يمكن التحقّق منه على رابط المرشّح:** المعاينة تعمل بـ
   `founder_preview`، فـ`isSupabaseConfigured()` كاذبة عمدًا (DEC-002) ولا
   سلطة كتابة لها. ولذلك **مسار البوّابة (تجربة · استرداد · مطالبة) لا
   يُختبَر هناك إطلاقًا** — يحتاج بيئة staging ببيئة Supabase حقيقية بعد
   استيفاء الشروط الثلاثة أعلاه. من يفتح المعاينة ولا يجد زرّ التجربة يعمل
   **لم يجد عطلًا**.

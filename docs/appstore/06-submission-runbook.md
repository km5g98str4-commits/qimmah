...
6. **قرار سنّ الأهلية** (Terms §3) — لا توجد بوابة عمر اليوم. ⚠ (راجع `03-age-rating.md`).
7. **علم التصميم**: قرّر هل تبني بـ `VITE_DESIGN_V2=true` (واجهة v2.1) — يجب أن يطابق اللقطات. ⚠
...
5. **Screenshots**: ارفع مجموعة **6.9″ (1290×2796)** بستّ لقطات حسب `04-screenshots-plan.md` (RTL، بيانات مصدّرة).
...
2. اتبع `docs/legal/app-privacy-labels.md` **حرفيًا**: التتبّع = لا؛ وأنواع البيانات «المجموعة» مقابل
   ...
   بلا تتبّع؛ التغذية/الماء/الخطوات على الجهاز = غير مجموعة).
3. **خانة التحليلات**: إن كانت نسخة البناء **لا** تضبط `VITE_ANALYTICS_ENDPOINT` → «Usage/Diagnostics = Not
...
- الكيان الناشر · فريق التوقيع · الأيقونة/الـSplash النهائية · نطاق الروابط القانونية العام ·
  سنّ الأهلية · شحن v2.1 أم v1 · ضبط `VITE_ANALYTICS_ENDPOINT` أم لا · الفئة الثانوية · روابط الدعم/التسويق ·

...
> **⚠ Superseded by the Consolidation resolution section below.** This table records the *earlier fix-wave* session's homegrown resolutions. The grand consolidation (HEAD `7b35435`+) later merged the **specialized** branches that carry deeper proofs, and per DEDUPE LAW those supersede the homegrown versions in two places: **secure-import** (specialized `f4b2f95` `DataManagementPanel` + 34-vector e2e replaced homegrown `0e293bf`) and **media** (specialized `1d4d64f` **24 IN-HOUSE / UNKNOWN=0** replaced the homegrown "keep 23 UNKNOWN, owner-acknowledged" variant `2abad8b`). **QEA-005 skip-link is now FIXED** (not out-of-scope). Read the Consolidation resolution table as authoritative; the rows below are kept for provenance.
...
| FITWILL asset + 23 UNKNOWN (MEDIA-RIGHTS.md) | P1/P2-class **CODE** | ✅ **FIXED** — ~~`2abad8b`~~ **superseded by `1d4d64f`** | **Consolidation (specialized) outcome:** all 24 unsafe assets (23 UNKNOWN + 1 RESTRICTED FITWILL) **replaced by in-house SVG schematics**. Manifest **274/274 — 250 CLEARLY-LICENSED + 24 IN-HOUSE, UNKNOWN=0, RESTRICTED=0.** (The earlier fix-wave `2abad8b` instead kept the 23 jpgs flagged `ownerAck: rights:unknown` and reported 273; DEDUPE LAW keeps the specialized full remediation.) Origin's independent stale `p3-media-proof` cable-biceps-curl fix retained. Media proofs green. |
...
| **QEA-005** skip-to-main-content link | P2 **CODE** | ✅ **FIXED** — `4b8f266` (`9c5520b`) | Consolidation merged `ux/core-product-polish`: `MobileShell` renders a first-focus `a[href="#main-content"]` ("تخطَّ إلى المحتوى / Skip to content") targeting `<main id="main-content" tabIndex={-1}>`, plus safe-area `--safe-*` paddings. (Earlier fix-wave had deferred this as out-of-scope; the consolidation closed it.) |
...
| Minors maintenance-only nutrition goals (Option B) | `730df00` (`fix/minors-maintenance-only`, `--no-ff` merge into promotion) | **`test:minors` 82/82 + formula now 159/159.** Under-18 → maintenance only: target==maintenance==TDEE for any stored goal (no deficit/surplus), no weight-change forecast, age-18 restores the deficit. Owner-scoped idempotent goal migration + one-time notice (wiped on account switch). Goal cards `disabled`+`aria-disabled` with AA-contrast restriction note in `StepBody` + `OnboardingV2`. Terms §3 AR+EN. Live browser sweep ×2 (MINOR/ADULT) zero console errors. |

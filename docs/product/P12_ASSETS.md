...
- **مُبقاة لكن مُعلَّمة للمراجعة** (ليست بار/بنش حرّ فخارج قاعدة الحذف، لكن اللقطة ليست الجهاز):
  `shoulder-press-machine` (ضغط كتف بحبل مقاومة) و`standing-leg-curl` (ثني ساق واقف بوزن الجسم).
  تركتها بانتظار قرارك.
- بعد الحذف: `node scripts/p12-sync-gifs.mjs` أعاد توليد `exerciseGifs.ts` (84 مدخلًا) — المعرّفات
  الثمانية لم تعد مرتبطة بأي ملف فتعرض البديل الأنيق.

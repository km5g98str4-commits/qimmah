...
- **حادثة 2026-07-03:** تشغيل سابق افترض endpoint بحث لكل تمرين (`?search=`) غير موجود → 46 طلبًا احترقت كلها بـ404 وصفر تنزيلات. **المتبقي مدى الحياة ≈ 229 طلبًا.**
- **الشكل الصحيح (المُثبت من P5):** طلب قائمة واحد `GET /exercises` + مطابقة محلية + تنزيل من CDN بلا مفتاح.
- المخطّط الآن: **probe = 1 طلب، ثم التشغيل الكامل = 1 طلب قائمة** (أسوأ حالة 4 إن جُرّبت المسارات الاحتياطية) + 46 تنزيل CDN لا يحمل المفتاح — **إجمالي API متوقع ≈ 2، أسوأ حالة ≈ 5 ≤ السقف الصارم 150 ≤ المتبقي 229** ✅

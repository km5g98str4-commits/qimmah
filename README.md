...
## 🎛️ 3.5) مركز التخصيص (Customization Center)

نسخة أولية من واجهة تتيح للمشتري تعديل بيانات القالب **بصريًا** دون فتح الكود.

### أين أجده؟
- **الزر العائم** أسفل يمين الصفحة: «مركز التخصيص».
- أو زر **«افتح مركز التخصيص»** داخل قسم التخصيص في الصفحة الرئيسية.

### ماذا يمكن تعديله؟
الهوية (اسم المستخدم/المدرب · الشعار النصي · الهدف الرئيسي · نوع المستخدم: فرد/مدرب/صانع محتوى) · اللون الأساسي ولون التمييز · جدول التمارين · المكملات · الوجبات · القياسات · الروتين الأسبوعي — مع **معاينة حية** للهوية والألوان.

### كيف يعمل؟
- الأزرار: **حفظ التخصيص** (يخزّن في `localStorage`) · **استعادة الافتراضي** (يمسح المحفوظ ويعيد القيم من `config`/`data`).
- القيم الافتراضية تُقرأ مباشرة من ملفات `config`/`data` — مصدر الحقيقة الوحيد.

### ⚠️ حدود النسخة الحالية (بدون backend)
- **لا تسجيل دخول ولا خادم** — التعديلات تُحفظ في متصفح هذا الجهاز فقط، ولا تنتقل لأجهزة أخرى ولا تُنشر للزوّار.
- المركز أداة **تحرير ومعاينة**؛ التخصيص **الدائم** في الإنتاج يكون بتعديل ملفات `config`/`data` يدويًا (الأقسام 2 و3 أعلاه).
- ربط المركز ببيانات حيّة لكل المكونات + حسابات فعلية = خطوة لاحقة تتطلب backend (انظر `ROADMAP.md`).

> 💡 سير عمل مقترح: جرّب التعديلات في المركز → انسخ القيم النهائية → ثبّتها في `config`/`data` لتصبح دائمة في النسخة المنشورة.

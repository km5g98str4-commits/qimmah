...
>
> تاريخ البناء: `2026-09-12T18:01:01.072Z` · نسخة التطبيع: `1.1.0` · نسخة المخطّط: `1.0.0`
...
| المقياس | القيمة |
|---|---|
| **سجلات مقبولة ومشحونة** | **62,939** |
| GTIN فريد صالح | 62,939 |
| السوق السعودي (SA) | 5,320 |
| الخليج (GCC) | 2,567 |
| عالمي (GLOBAL) | 55,052 |
| مكرّرات أُزيلت | 1,715 |
| تعارضات مرفوعة (لم تُدمج) | 2,080 |
| طابور مراجعة ضبابي | 5,313 |

> التصدير نُزّل كاملًا (0.00 ميغابايت) وقُرئ حتى آخره.

**مقابل الهدف (٤٠٬٠٠٠):** تحقّق — 62,939 سجلًا.
...
السقف ليس ندرة البيانات بل **سقف شحن مقصود**: 1,881,142 سجلًا
...
| سبب الرفض | العدد |
|---|---|
| `missing_energy` | 1,742,580 |
| `missing_macros` | 1,724,954 |
...
قُرئ 4,535,553 صفًّا من التصدير. الرفض **مسمّى دائمًا** ولا يُبتلع صفٌّ صامتًا.
...
|---|---|---|
| اسم عربي | 880 | 1.4% |
| اسم إنجليزي | 62,454 | 99.2% |
| ماكروز كاملة (بروتين+كارب+دهن) | 62,549 | 99.4% |
| صوديوم | 57,489 | 91.3% |
| ألياف | 51,258 | 81.4% |
| حجم حصّة | 37,773 | 60.0% |
| مكوّنات | 53,378 | 84.8% |

### توزيع الثقة

- `high_0.8+` — 48,594 (77.2%)
- `mid_0.6-0.8` — 10,244 (16.3%)
- `low_0.4-0.6` — 3,264 (5.2%)
- `very_low_<0.4` — 837 (1.3%)

### أعلام الجودة (وسم لا رفض)

| العلم | العدد | النسبة |
|---|---|---|
| `no_arabic_name` | 62,059 | 98.6% |
| `no_brand` | 2,929 | 4.7% |
| `fiber_exceeds_carbs` | 1,528 | 2.4% |
| `macro_energy_mismatch` | 1,499 | 2.4% |
| `no_english_name` | 485 | 0.8% |
| `sugar_exceeds_carbs` | 95 | 0.2% |
| `saturated_exceeds_fat` | 40 | 0.1% |
| `serving_size_implausible` | 5 | 0.0% |
| `serving_basis_suspect` | 3 | 0.0% |
...
|---|---|---|---|
| **الخليج والسعودية** | 7,887 | 760 | **9.6%** |
| ‏— منها من Open Food Facts | 7,705 | 578 | 7.5% |
| ‏— منها من تنسيق قِمّة | 182 | 182 | 100.0% |
| **الذيل العالمي** | 55,052 | 120 | **0.2%** |

**الرقم الإجمالي (1.4%) مضلِّل ولا يُقرَأ وحده.** فهو متوسّط ممزوج:
الذيل العالمي أوروبي في غالبه فتغطيته العربية 0.2%، بينما مجموعة الخليج
—وهي ما يهمّ المستخدم السعودي فعلًا— تغطيتها **9.6%**.
...
> كتلة السعودية ٦٢٨ بعد. بعد اكتماله تبيّن أن OFF يقدّم 578 اسمًا عربيًا في الخليج.
...
|---|---|
| عدد الشرائح | 44 |
| أصغر شريحة (مضغوطة) | 288 كيلوبايت |
| الوسيط | 311 كيلوبايت |
| أكبر شريحة | 324 كيلوبايت |
| **ضمن الميزانية (٢٠٠–٥٠٠ ك.ب)** | **44/44** |
| بيانات مضغوطة | 13.28 ميغابايت |
| بيانات خام | 73.25 ميغابايت |
| فهارس البحث مضغوطة | 2.70 ميغابايت |
| **الإجمالي المضغوط** | **15.97 ميغابايت** |
| الطقم الساخن | 600 سجلًا · 86 ك.ب مضغوط · ميزانية 120 ك.ب · **ضمنها** |

### لماذا هذا الشكل

- **البحث بالباركود مفتاح مباشر:** `shard = (mix32(fnv1a(gtin14)) mod shard_count), where mix32 is the MurmurHash3 32-bit finalizer. The finalizer is REQUIRED, not cosmetic: every valid GTIN has an even digit sum (a consequence of the mod-10 check digit), which makes every raw FNV-1a hash odd and leaves half the shards permanently empty. Runtime and pipeline import this one module — see src/lib/food/shardRouting.ts.` ⇒ شريحة واحدة ⇒ مفتاح O(1).
...
- **الشرائح لا تُلتزم في git** (73.25 ميغابايت): تُولَّد حتميًا وبصمة كلٍّ منها
...
1. **تغطية العربية 1.4%** — تُعالَج بالطقم الساخن المنسَّق،

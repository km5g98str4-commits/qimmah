// Runner for question-oracle-adapter.ts — esbuild @→src, localStorage shim.
...
const result = await build({
  entryPoints: [resolve(__dirname, 'question-oracle-adapter.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const outDir = mkdtempSync(join(tmpdir(), 'qae-qoracle-'))
const bundlePath = join(outDir, 'question-oracle.bundle.mjs')

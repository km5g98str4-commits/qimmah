// Runner for extract-legacy-catalog.ts — esbuild @→src, localStorage shim.
...
const result = await build({
  entryPoints: [resolve(__dirname, 'extract-legacy-catalog.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const outDir = mkdtempSync(join(tmpdir(), 'qae-catalog-'))
const bundlePath = join(outDir, 'extract-catalog.bundle.mjs')

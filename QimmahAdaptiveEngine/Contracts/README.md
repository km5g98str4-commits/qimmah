...
| bank: L-QST-7 revival + Policy B | [CTO-QAE-007] §1–§2 | `b-target-weight` revived (dead leaf → `primaryGoalDisplay`, age≥18 kept, not required; active 153→154) · `x-return-reason`/`x-return-ramp` required-when-eligible (returning users only) | plan goldens byte-identical; non-returning journey STEPS byte-identical (bankSize/bankAvoidedBp metadata only); returning journey asks the pair in the required stage (intended; displacement finding F2 in QAE-PHASE5-WAVE1-REPORT §0) |
| training contracts v1 | [CTO-QAE-007] Wave 1 | `Contracts/exercises/` (metadata schema + catalog 181, catalogManifestHash, blocklist-migration 11 rules) + `Contracts/training/` (capability schema + approved-deviations D1–D5) + `Domain/Catalog` + `Domain/Training` | new surface — 17 training goldens + qae-training-proof (131 checks) are its conformance suite; legacy ⊆ new proved, no silent loss |

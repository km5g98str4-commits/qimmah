...
function assertOfficialBaseline() {
  const actual = execFileSync('git', ['rev-parse', 'origin/main'], { cwd: root, encoding: 'utf8' }).trim()
  if (actual !== sourceCommit) fail(`origin/main is ${actual}; expected official baseline ${sourceCommit}`)
  const corePaths = [
  ...
  const diff = execFileSync('git', ['diff', '--name-only', 'origin/main', '--', ...corePaths], { cwd: root, encoding: 'utf8' }).trim()
  if (diff) fail(`Local core source differs from origin/main:\n${diff}`)
}

// يحزم إثبات تقويم التمرين الأسبوعي (TS بمسارات @/) عبر esbuild فوق localStorage
// مُحاكى ويشغّله (بلا متصفح) — نمط run-today-v2-model-proof.mjs نفسه.
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/workout-calendar-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'workout-calendar-'))

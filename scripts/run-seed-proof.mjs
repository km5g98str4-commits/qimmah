// يُجمّع إثبات البذرة (TS بمسارات @/ + استيراد demo-seed) عبر esbuild فوق
// localStorage مُحاكى ويشغّله (بلا متصفح/شبكة). لا يمسّ package.json.
//   node scripts/run-seed-proof.mjs
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/seed-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'seed-proof-'))

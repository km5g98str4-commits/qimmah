...
for (const ex of exercises) {
  ...
  if (cards[ex.id]) {
    // [FOUNDER-CARDS-001] بطاقة المؤسس تسبق كل الطبقات: صورة أصلية للتمرين نفسه بعنوانه.
    imageStatus = 'APPROVED'
    image = { kind: 'card', start: cards[ex.id], end: null }
    imageNote = 'Founder-produced exercise card (in-house render) — replaces diagram/illustration/archive stills for this movement.'
  } else if (img.status === 'stills' && img.stillStart) {
    imageStatus = 'APPROVED'
  ...
  entries[ex.id] = {
    ...
    image,
    imageStatus,
    imageSource: image?.kind === 'card' ? 'qimmah-founder-card' : image?.kind === 'illustration' ? 'qimmah-inhouse-illustration' : (img.source ?? null),
    imageLicense:
      image?.kind === 'card'
        ? 'Founder-produced original exercise card — Qimmah owns full rights'

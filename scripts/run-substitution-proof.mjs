// Bundles the substitution proof (TS with @/ paths) via esbuild over a simulated
// localStorage and runs it in node — no browser. Mirrors run-finish-confirm-proof.mjs.
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/substitution-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'substitution-'))

// يُشغّل إثبات مصدر الحقيقة الواحد (TS بمسارات @/) عبر esbuild فوق localStorage
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/profile-domain-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'profile-domain-'))

// مُشغّل إثبات «وش بيتغيّر؟» (test:plan-changes) — [CTO-71] البند ٥.
// يُجمّع TS بمسارات @/ عبر esbuild فوق localStorage مُحاكى ويشغّله بلا متصفح.
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/plan-changes-proof.ts')],
...
const dir = mkdtempSync(join(tmpdir(), 'plan-changes-'))

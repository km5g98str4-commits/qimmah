// يحزم إثبات ترتيب اليوم الأول ([SOVEREIGN-PLAN-003]) عبر esbuild فوق كعب متصفّح
// **كامل** ويشغّله بلا متصفح — نمط run-workout-calendar-proof.mjs نفسه.
...
// localStorage مكتمل المواصفات (length + key(i)) — النماذج تقرأ عدّة مفاتيح.
const banner = `
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/first-session-day-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'first-session-day-'))

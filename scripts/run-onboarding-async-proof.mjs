// يُجمّع إثبات تدفّق الإعداد v2.1 (TS بمسارات @/) عبر esbuild فوق localStorage
// مُحاكى ويشغّله (بلا متصفح). نفس نمط run-active-session-proof.mjs.
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/onboarding-async-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'onboarding-async-'))

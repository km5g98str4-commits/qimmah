 ...
 *  «باسكن» و«غايز» أضافتهما موجة [مهمة الصقل §5] بنفس العقد المقيس. */
const WAVE_LOANWORDS = new Set(['ماسترو', 'دانكن', 'مارغريتا', 'باسكن', 'غايز'])
...
// [FOOD-GENERIC-001] الأصناف العامّة المولَّدة (gen-* · bakery-* · rst-*) لاحقة على هذه الموجة أيضًا — تُستبعد من «القديم».
const LEGACY_ITEMS = foodItems
  .filter((f) => !WAVE_ITEM_IDS.has(f.id) && !/^(gen|bakery|rst)-/.test(f.id))
  .map((f) => {
...
counter(
  'ونزعُ مقابلة «باسكين⇒باسكن» يعيدها إلى صفر',
  legacySearch('باسكين').length === 0 && unified.rankCurated('باسكين').length > 0,
  `قديم ${legacySearch('باسكين').length} · جديد ${unified.rankCurated('باسكين').length}`,
)
counter(
  'ونزعُ مقابلة «قايز⇒غايز» يعيد «فايف قايز» إلى صفر',
  legacySearch('فايف قايز').length === 0 && unified.rankCurated('فايف قايز').length > 0,
  `قديم ${legacySearch('فايف قايز').length} · جديد ${unified.rankCurated('فايف قايز').length}`,
)
/**
 * ⟲ حارس التعميم المحظور (§4.2): «غايز≡قايز» كلمة بعينها لا قاعدة قلب قاف/غين.
 * الشاهدة الأصيلة «قشطة» (٤ أصناف حقيقية) — لو تسلّل قلبٌ معمَّم لصارت «غشطة»
 * تجدها. بقاء «غشطة» صفرًا مع بقاء «قشطة» واجدةً هو البرهان المسمّى.
 */
counter(
  'و«غشطة» تبقى صفرًا بينما «قشطة» واجدة — التكافؤ كلمة لا قاعدة قاف/غين',
  unified.rankCurated('غشطة').length === 0 && unified.rankCurated('قشطة').length > 0,
  `غشطة ${unified.rankCurated('غشطة').length} · قشطة ${unified.rankCurated('قشطة').length}`,
)
counter(

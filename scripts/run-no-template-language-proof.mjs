...
const allSurfaceCopy = SURFACES.map(([, source]) => source).join('\n')
const LEGITIMATE = [
...
for (const [label, re] of LEGITIMATE) {
  // النص قد يعيش في قاموسه المتخصص لا في strings.ts التاريخي؛ العقد هو بقاؤه
  // في سطح واجهة حي، وSURFACES أعلاه هو نفس النطاق الذي يفحصه الحارس للحظر.
  check(`${label} — باقٍ ولم يُمسح بالجملة`, re.test(allSurfaceCopy))
}

...
try {
  ...
  check('السجل يعرض القيم الحقيقية ووسم الدهون التقديري', await page.getByText('81.5', { exact: false }).isVisible() && await page.getByText(/نسبة الدهون · تقديري/).isVisible())
  await page.reload({ waitUntil: 'networkidle' })
  await page.locator('[data-testid="measurements-history"]').waitFor()
  check('السجل يبقى بعد reload', await page.getByText('81.5', { exact: false }).isVisible())

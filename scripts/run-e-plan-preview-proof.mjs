// يشغّل إثبات معاينة الخطة (حارة E) عبر esbuild وreact-dom/server — بلا متصفح.
// node using react-dom/server — no browser. A minimal window/localStorage shim
// keeps any incidental imports happy.
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/e-plan-preview-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  jsx: 'automatic',
...
const dir = mkdtempSync(join(tmpdir(), 'e-plan-preview-'))

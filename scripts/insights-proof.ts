...
function baseInput(over: Partial<InsightInput> = {}): InsightInput {
  return { nowMs: NOW, lang: 'ar', sessions: [], plan: { daysPerWeek: 4, targetProtein: 160, muscleGroups: ['الصدر', 'الظهر', 'الأرجل'] }, weights: [], proteinByDate: {}, prBests: {}, ...over }
}
...
const noTarget = computeAllMetrics(baseInput({ plan: { daysPerWeek: 0, targetProtein: null, muscleGroups: [] } }))
...
const planSplit = computeAllMetrics(baseInput({
  plan: { daysPerWeek: 2, targetProtein: 160, muscleGroups: ['الصدر', 'الظهر'] },
  sessions: [
    { date: stamp(NOW - DAY), volume: 1000, muscleGroups: ['الصدر'], topSets: [] },
    { date: stamp(NOW - 2 * DAY), volume: 900, muscleGroups: ['الصدر'], topSets: [] },
  ],
})).muscleSplit
ok('توزيع العضلات يقارن عضلات الخطة فقط', planSplit.status === 'ok' && planSplit.undertrained.length === 1 && planSplit.undertrained[0] === 'الظهر')

// ——— 6) غياب المتاجر (مدخل فارغ) ⇒ امتناع كامل ———
console.log('— غياب المتاجر —')
const empty = buildInsightsFromInput(baseInput({ plan: { daysPerWeek: 0, targetProtein: null, muscleGroups: [] } }))
...
const prIn = buildInsightsFromInput(baseInput({
  sessions: [{ date: stamp(NOW - DAY), volume: 4000, muscleGroups: ['الصدر'], topSets: [{ exerciseId: 'bench', name: 'بنش', weightKg: 96 }] }],
  prBests: { bench: { name: 'بنش', best: 100 } },
}))
...
const prFar = computeAllMetrics(baseInput({
  sessions: [{ date: stamp(NOW - DAY), volume: 4000, muscleGroups: ['الصدر'], topSets: [{ exerciseId: 'bench', name: 'بنش', weightKg: 70 }] }],
  prBests: { bench: { name: 'بنش', best: 100 } },
})).pr
...
const rich = baseInput({
  weights: weightSeries(82, -0.15, 8),
  sessions: [
    { date: stamp(NOW - DAY), volume: 6000, muscleGroups: ['الصدر', 'الظهر'], topSets: [{ exerciseId: 'bench', name: 'بنش', weightKg: 96 }] },
...
const seedWeights = (arr: { date: string; kg: number }[]) => LS.setItem('qimmah:history:measurementLogs:v1', JSON.stringify(arr.map((w, i) => ({ id: 'm' + i, date: w.date, values: { weightKg: w.kg } }))))

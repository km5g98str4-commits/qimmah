// يُجمّع إثبات حفظ/استئناف الجلسة النشطة (TS بمسارات @/) عبر esbuild فوق
// localStorage مُحاكى ويشغّله (بلا متصفح).
...
// localStorage مُحاكى مطابق للمواصفة (بما فيه length + key(i)) — يحتاجه wipeUserData
// الذي يمسح بالمسح على البادئة.
const banner = `
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/isolation-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,

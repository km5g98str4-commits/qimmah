// Bundles the finish-confirm proof (TS with @/ paths) via esbuild over a
// simulated localStorage that implements length/key (needed by the snapshot
// helper), and runs it in node — no browser.
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/finish-confirm-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'finish-confirm-'))

// يحزم إثبات محرّك حالة الجلسة (TS بمسارات @/) عبر esbuild فوق localStorage
...
// localStorage مكتمل المواصفات (length + key(i)) — لقطة التراجع تعدّد المفاتيح.
const banner = `
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/session-engine-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  write: false,
...
const dir = mkdtempSync(join(tmpdir(), 'session-engine-'))

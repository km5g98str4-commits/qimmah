// Bundles the StateBlock proof (TSX with @/ paths) via esbuild and runs it in
...
const result = await build({
  entryPoints: [resolve(root, 'scripts/state-block-proof.ts')],
  bundle: true,
  format: 'esm',
  platform: 'node',
  jsx: 'automatic',
...
const dir = mkdtempSync(join(tmpdir(), 'state-block-'))

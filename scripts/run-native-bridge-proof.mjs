...
const result = await build({
  entryPoints: [resolve(root, 'scripts/native-bridge-proof.ts')], bundle: true, format: 'esm', platform: 'node', write: false,
  banner: { js: banner }, alias: { '@': resolve(root, 'src') },
  define: {
    'import.meta.env': JSON.stringify({ MODE: 'test', DEV: false, PROD: false }),
  }, logLevel: 'warning',
})
const file = join(mkdtempSync(join(tmpdir(), 'native-bridge-')), 'proof.mjs')
